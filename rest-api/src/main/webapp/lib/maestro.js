...
function addExampleButtons(){
    ...
    var exampleOpsAndButton = "<select class='example-operations-select'><option value='uk.gov.gchq.maestro.operation.OperationChain'>OperationChain</option></select>" + exampleButton;
    var exampleHtml;
    if(getVersion() === "v1") {
...
function loadExample(exampleButton){
    ...
    if(version === "v1") {
        ...
        if (classPathParam == "" && urlSuffix.includes('{')) {
            classPathParam = "uk.gov.gchq.maestro.operation.OperationChain"
            row.children[1].firstElementChild.value = "uk.gov.gchq.maestro.operation.OperationChain"
        }
...
function hideJobsIfRequired() {
    $.get(getVersion() + '/graph/operations/uk.gov.gchq.maestro.operation.impl.job.GetJobDetails')
    .fail(function() {
...
function updateTitle(properties) {
    updateElement('maestro.properties.app.title', properties, function(value, id) {
        $('#' + id).text(value);
...
function updateFooter(properties) {
    updateElementWithId("swagger-ui-container", "maestro.version", properties, function(value, id) {
        var footer = $('#' + id).find(".footer")
        var footerText = footer.text();
        var newFooterText = footerText.substring(0, footerText.indexOf("]")) + ", maestro version: " + value + " ]";
...
function updateBanner(properties) {
    if($('#banner').length == 0) {
        updateElementWithId('banner', 'maestro.properties.app.banner.description', properties, function (value, id) {
            $('body').prepend("<div id='banner' class='banner'>" + value + "</div>")
            updateElementWithId('banner', 'maestro.properties.app.banner.colour', properties, function(value, id) {
...
function updateDescription(properties) {
    updateElement('maestro.properties.app.description', properties, function(value, id) {
        $('#' + id).html(value);
    });
}

function updateDocUrl(properties) {
    updateElementWithId('doc-url', 'maestro.properties.app.doc.url', properties, function(value, id) {
        $('#' + id).html("For more information see our <a href='" + value + "'>documentation</a>.");
    });
}

function updateLogoLink(properties) {
    updateElementWithId('logo', 'maestro.properties.app.logo.link', properties, function(value, id) {
        $('#' + id).attr("href", value);
    });
}
...
function updateLogo(properties) {
    if(!$('#logo img').hasClass('logo__img')) {
        updateElementWithId('logo', 'maestro.properties.app.logo.src', properties, function(value, id) {
...
function updateFavicon(properties) {
    if(!$('head').children().hasClass('favicon-16')) {
        updateElement('maestro.properties.app.logo.favicon.small', properties, function(value, id) {
                $('head').prepend("<link class='favicon-16' rel='icon' type='image/png' href='" + value + "' sizes='16x16'/>");
        });
    }
    if(!$('head').children().hasClass('favicon-32')) {
        updateElement('maestro.properties.app.logo.favicon.large', properties, function(value, id) {
