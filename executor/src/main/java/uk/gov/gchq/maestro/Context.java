...
public class Context {
     ...
     */
    private final Map<String, Operation> exporters = new HashMap<>();
    ...
    public Collection<Operation> getExporters() {
        return Collections.unmodifiableCollection(exporters.values());
    }

    public void addExporter(final Operation exporter) {
    ...
    public  Operation getExporter(final String exporterId) {
        if (null == exporterId) {
    ...
    public OperationChain getOriginalOpChain() {
        return OperationChain.wrap(originalOperation.getId(), originalOperation);
    }
