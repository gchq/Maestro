...
package uk.gov.gchq.maestro.executor.operation.handler.named.cache;
 ...
 * the {@link uk.gov.gchq.maestro.operation.operation.NamedOperation}s for a Maestro
 * graph.
 */
public class NamedOperationCache {
     ...
     * @throws CacheOperationException thrown if the user doesn't have write access to the
     *                                 NamedOperationDetail requested,
     ...
     *
     * @throws CacheOperationException if there was an error clearing the
     *                                 cache
     */
     ...
     * {@link uk.gov.gchq.maestro.operation.operation.NamedOperation} from the
     * cache.
     *
     * @param name the name of the operation to delete
     * @throws CacheOperationException if there was an error deleting the
     *                                 operation from the cache
     */
    public void deleteFromCache(final String name) throws CacheOperationException {
        ...
        if (null != CacheServiceLoader.getService().getFromCache(CACHE_NAME, name)) {
            throw new CacheOperationException("Failed to remove " + name + " from cache");
        }
    }
     ...
     *                  provided name
     * @throws CacheOperationException if there was an error adding the
     *                                 operation to the cache
     */
    public void addToCache(final String name, final NamedOperationDetail operation, final boolean overwrite) throws CacheOperationException {
        try {
            if (overwrite) {
    ...
    public NamedOperationDetail getFromCache(final String name) throws CacheOperationException {
        ...
        throw new CacheOperationException("No named operation with the name " + name + " exists in the cache");
    }

    private void add(final NamedOperationDetail namedOperation, final boolean overwrite, final User user, final String adminAuth) throws CacheOperationException {
        String name;
        try {
        ...
        if (existing.hasWriteAccess(user, adminAuth)) {
            ...
            throw new CacheOperationException("User " + user.getUserId() + " does not have permission to overwrite");
        }
    }

    private void remove(final String name, final User user, final String adminAuth) throws CacheOperationException {
        if (null == name) {
            throw new CacheOperationException("NamedOperation name cannot be null");
        }
        ...
        if (existing.hasWriteAccess(user, adminAuth)) {
            ...
            throw new CacheOperationException("User " + user +
                    " does not have authority to delete named operation: " + name);
        }
    }

    private NamedOperationDetail get(final String name, final User user, final String adminAuth) throws CacheOperationException {
        final NamedOperationDetail op = getFromCache(name);
        if (op.hasReadAccess(user, adminAuth)) {
            return op;
        } else {
            throw new CacheOperationException("User: " + user + " does not have read access to " + name);
        }
    }
